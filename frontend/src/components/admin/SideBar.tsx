...
const SideBar = () => {
  ...
  const logout = async () => {
    try {
      setIsLoader(true);
      // Use the authService logout function which handles Redux state and localStorage
      await authLogout();
      setIsLoader(false);
      // Navigate to home page after successful logout - use window.location for full page reload
