...
import { editSuccessResponseMessage } from '@/utils/responseMessages';
...
const EditNews = () => {
  ...
  useEffect(() => {
    ...
    // Set the image preview if image_url exists
    if (postData.image_url) {
      setImage(postData.image_url);
      // Create a dummy file for form state
      const dummyFile = new File([''], 'image.jpg', { type: 'image/jpeg' });
      setValue('image', [dummyFile]);
    }
  }, [setValue, posts]);
  ...
  const onSubmit: SubmitHandler<NewsFormInput> = async (values) => {
    if (!id) return;
    try {
      setIsProcessing(true);
      const resultAction = await dispatch(editPost({ id, values }));
      ...
      dispatch(openAlert('Сталася несподівана помилка при оновленні новини'));
    } finally {
